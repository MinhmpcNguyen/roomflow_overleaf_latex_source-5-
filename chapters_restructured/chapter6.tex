...
This organization keeps pure planning and solving logic separate from HTTP and database concerns. It also makes ablation testing practical because individual modules can be disabled or replaced without changing the entire API surface. Figure~\ref{fig:6-2} shows the package dependency direction used to preserve this separation across the API layer, application services, domain modules, infrastructure and tools, and evaluation utilities. It also shows that API routes depend on application services, while services call domain modules such as room interpretation, style and material handling, cluster planning, layout solving, and gallery selection. Persistence, configuration, artifact storage, external clients, and evaluation tools stay at the infrastructure boundary.
...
The renderer supports three user-facing modes: generating a new render from a selected 3D snapshot, editing an existing render with a localized request, and re-rendering from a different camera view. The evaluation code can additionally run ablation variants of the generation workflow, such as text-only prompting, text contracts, visual preflight, and locked-geometry references. These variants are used for Chapter 8 experiments, while the user-facing workflow exposes only the selected generation and editing controls. Figure~\ref{fig:6-4} shows the sequence used when a selected layout, camera view, and visual preset are converted into a 3D snapshot, a provider request, and a saved render artifact.
...
Figure~\ref{fig:6-5} presents the simplified database E-R diagram for these runtime entities. The relational schema in Figure~\ref{fig:6-5} keeps stable ownership and traceability fields in columns, while flexible layout and render payloads are stored as JSONB. This is suitable for a prototype whose layout schema evolves during experiments but still needs account-level isolation and reproducible links between layouts and renders. Table~\ref{tab:6-1} lists the main entities, their key fields, and how each entity supports authentication, inventory management, saved layouts, generated renders, and reusable design knowledge.
...
\hline
\textbf{Entity/table} & \textbf{Key fields} & \textbf{Main relationship} & \textbf{Role in the system} \\
\hline
...
The project uses a web stack and AI-service integration rather than a single monolithic desktop application. Table~\ref{tab:6-2} summarizes the main tools and libraries used in the implemented system. The table separates frontend interaction, 2D/3D visualization, backend APIs, persistence, AI services, geometry evaluation, and deployment so that each implementation layer can be traced to its technical role.
...
\hline
\textbf{Purpose} & \textbf{Tool or library} & \textbf{Role in the project} \\
\hline
...
\hline
\textbf{Item} & \textbf{Delivered artifact} & \textbf{Role in the system} \\
\hline
...
The following figures illustrate the main implemented functions from the user perspective. \hyperref[fig:6-6]{Figure~6.6} shows the full workspace and the relationship between the inventory panel, editable floor plan, design brief, and generation controls. Figures~\ref{fig:6-7}, \ref{fig:6-8}, and \ref{fig:6-9} enlarge the object library, room editor, 3D preview, and render controls so that the implemented controls remain readable on a portrait thesis page. These figures show that manual editing, automatic layout generation, camera control, and rendering are exposed in the same workspace instead of being separate scripts.
...
Figure~\ref{fig:6-10} shows the generated perspective render beside its 3D layout source, making the connection between the editable scene and the visualization output visible in the implemented interface. This final screenshot confirms that the implementation keeps the layout and render artifacts side by side, which supports the thesis goal of layout-consistent visualization.
...
\hline
\textbf{Component} & \textbf{Typical execution} & \textbf{Configuration} & \textbf{Security or storage note} \\
\hline
